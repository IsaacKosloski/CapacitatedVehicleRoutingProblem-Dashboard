...
def get_real_time(instance_name, method):
    """Obtém o tempo real dos arquivos .sol"""
    if instance_name in TIMES_DATA and method in TIMES_DATA[instance_name]:
        return TIMES_DATA[instance_name][method]
    return None
...
def format_number(num):
    """Formata número com ponto para milhares e vírgula para decimal"""
    if num >= 1000:
        ...
        if dec_part > 0:
            return f"{int_str},{dec_part:.2f}".split('.')[1]
        return int_str
    else:
        return f"{num:.2f}".replace('.', ',')

# Gerar tabela
print("Gerando tabela LaTeX corrigida com tempos reais...")
print()

rows = []
for instance in sorted(BEST_KNOWN.keys()):
    best_known = BEST_KNOWN[instance]

    # ILS-Swap
    ils_swap_best = get_best_result(instance, 'ILS-Swap')
    ils_swap_time = get_real_time(instance, 'ILS-Swap')

    # GRASP-Swap
    grasp_swap_best = get_best_result(instance, 'GRASP-Swap')
    grasp_swap_time = get_real_time(instance, 'GRASP-Swap')

    if ils_swap_best and grasp_swap_best and ils_swap_time and grasp_swap_time:
        ils_gap = calculate_gap(best_known, ils_swap_best)
        grasp_gap = calculate_gap(best_known, grasp_swap_best)

        # Formatar números
        bks_fmt = format_number(best_known)
        ils_res_fmt = format_number(ils_swap_best)
        grasp_res_fmt = format_number(grasp_swap_best)

        row = f"{instance:12} & {bks_fmt:>10} & {ils_res_fmt:>11} & {ils_gap:>6.2f} & {ils_swap_time:>6.2f} & {grasp_res_fmt:>11} & {grasp_gap:>6.2f} & {grasp_swap_time:>6.2f} \\\\"
        rows.append(row)

# Salvar em arquivo
output = """A Tabela \\ref{tab:ils_grasp_swap_comparison}, de resultados, compara o desempenho dos algoritmos ILS e GRASP utilizando o Swap* em todas as 100 instâncias do conjunto X. Ela apresenta:
\\begin{itemize}
...
with open('tabela_swap_corrigida.tex', 'w', encoding='utf-8') as f:
    f.write(output)

print(f"✅ Tabela salva em: tabela_swap_corrigida.tex")
print(f"   Total de linhas: {len(rows)}")

# Mostrar amostra
print("\n📊 Amostra da tabela (primeiras 5 linhas):")
print("-" * 120)
for row in rows[:5]:
    print(row)

print("\n📊 Amostra da tabela (últimas 5 linhas):")
print("-" * 120)
for row in rows[-5:]:
    print(row)
